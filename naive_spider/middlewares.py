...
import random
from .basic.design_mode import *


class NaivespiderMiddleware:
...
class NaivespiderDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
...
# class ProxyMap(Singleton):
#     def get_proxy(self, name):
#         if not hasattr(self, 'd'):
#             self.d = {}
#         proxy = self.d.get(name)
#         if proxy is None:
#             proxys = {k: int(v) for k, v in get_redis('ProxyIp').hgetall('proxy').items() if v != '0'}
#             proxy = min(proxys, key=lambda k: proxys[k])
#             self.d[name] = proxy
#         return proxy
    
#     def get_random_proxy(self):
#         proxys = [k for k, v in get_redis('ProxyIp').hgetall('proxy').items() if v != '0']
...
#     def process_request(self, request, spider):
#         # proxy = ProxyMap().get_proxy(spider.name)
#         if const.PROXY and not request.url.startswith(DATABASE_CRUD_URL):
#             proxy = ProxyMap().get_random_proxy()
#             # spider.logger.info('use proxy:', proxy)
#             # spider.clog.info('use proxy: %s' % proxy)
...
#             response.status not in self.retry_http_codes:
#             return response
...
class RandomUserAgentMiddleware:
    def process_request(self, request, spider):
        ...
        if user_agent and 'Scrapy' not in str(user_agent):
            # spider.clog.info('user-agent has already existed in request header.')
            pass
        else:
            request.headers['User-Agent'] = random.choice(user_agent_list)
            # spider.clog.info('set User-Agent to be %s when request the url: %s' % \
